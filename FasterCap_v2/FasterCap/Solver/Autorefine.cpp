...
//	m_ulNumofFastPotest = 0;
	...
	m_fDurationDiscretize = (float)(finish - start);
...
{
	FILE *stream = NULL;
	FILE *stream0 = NULL, *stream1 = NULL, *stream2 = NULL, *stream3 = NULL, *stream4 = NULL, *stream5 = NULL, *stream6 = NULL, *stream7 = NULL;
	int numwritten0, numwritten1, numwritten2, numwritten3, numwritten4, numwritten5, numwritten6, numwritten7;
	unsigned long j, k, start;
	wxFileName tmpFileName;
	int numwritten, ret;
	unsigned long chunksize, startbit, endbit, chunknum;
	int threadsbreak;
	...
	for(j=0; j< m_ulLinkChunkNum[m_ucInteractionLevel]; j++) {
		...
		if (multithreading == 1)
			{
				stream0 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream1 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream2 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream3 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream4 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream5 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream6 = fopen(tmpFileName.GetFullPath().c_str(), "rb");
				stream7 = fopen(tmpFileName.GetFullPath().c_str(), "rb");

				if(( stream0 == NULL ) || ( stream1 == NULL ) || ( stream2 == NULL ) || ( stream3 == NULL ) || ( stream4 == NULL ) || ( stream5 == NULL ) || ( stream6 == NULL ) || ( stream7 == NULL )) {
					ErrMsg("Error: cannot open the temporary file '%s' for reading\n", (const char*)tmpFileName.GetFullPath());
					return FC_FILE_ERROR;
				}

				chunksize = AUTOREFINE_LINK_CHUNK_SIZE / 8;
				threadsbreak = 0;

				#pragma omp parallel sections num_threads(8) private(k, chunknum, startbit, endbit) shared(chunksize, threadsbreak)
					{
						#pragma omp section
							{
								chunknum = 0;
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream0, startbit * sizeof( CAutoPanel* ), SEEK_SET);

								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten0 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream0 );
									if((numwritten0 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								...
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream1, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten1 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream1 );
									if((numwritten1 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								...
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream2, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten2 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream2 );
									if((numwritten2 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								...
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream3, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten3 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream3 );
									if((numwritten3 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								...
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream4, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten4 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream4 );
									if((numwritten4 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								...
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream5, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten5 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream5 );
									if((numwritten5 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								...
								startbit = chunksize * chunknum;
								endbit = chunksize * (chunknum + 1);
								fseek(stream6, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten6 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream6 );
									if((numwritten6 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
						...
						#pragma omp section
							{
								chunknum = 7;
								startbit = chunksize * chunknum;
								endbit = AUTOREFINE_LINK_CHUNK_SIZE;
								fseek(stream7, startbit * sizeof( CAutoPanel* ), SEEK_SET);
								
								// and read the current chunk from disk
								for(k = startbit; k < endbit; k++) {
									numwritten7 = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream7 );
									if((numwritten7 != 1) || (threadsbreak == 1)) {
										numwritten = 0;
										threadsbreak = 1;
										break;
									}
				...
				if(threadsbreak == 1) {
					if(numwritten == 0) {
						if(feof(stream0) || feof(stream1) || feof(stream2) || feof(stream3) || feof(stream4) || feof(stream5) || feof(stream6) || feof(stream7)) {
		...
		else
			{
				...
				// and read the current chunk from disk
				for(k = 0; k < AUTOREFINE_LINK_CHUNK_SIZE; k++) {
					numwritten = fread( &m_pdPanelPtrLinks[m_ucInteractionLevel][j][k], sizeof( CAutoPanel* ), 1, stream0 );
					// printf("%p", m_pdPanelPtrLinks[m_ucInteractionLevel][j][k]);
					if(numwritten != 1) {
						if(feof(stream0)) {
							ErrMsg("Error: unexpected end-of-file of temporary out-of-core file, stopping the process\n");
						}
...
int CAutoRefine::ReadFastCapFile(CAutoRefGlobalVars *globalVars)
{
	...
	finish = omp_get_wtime();
	m_fDurationReadFile = (float)(finish - start);
...
int CAutoRefine::BuildSuperHierarchy()
{
	...
	finish = omp_get_wtime();
	m_fDurationSuperH = (float)(finish - start);
